...
namespace QuantLab.BitCoin.Exchanges.API.Common
{
    /// <summary>
    ...
    public class SignalrManager
    {
        ...
        private void AddListener(string functionName, Action<string> callback, object[][] param)
        {
            ...
            ReconnectLoop().ContinueWith((t) =>
            {
                lock (listeners)
                {
                    ...
                    Console.WriteLine($"listeners.count={listeners.Count} ++++++++++++++++");
                }
        ...
        private void RemoveListener(string functionName, Action<string> callback)
        {
            lock (listeners)
            {
                ...
                Console.WriteLine($"RemoveListener.Count ==  {listeners.Count}+++++++++++++++++++++++");
        ...
        private void HandleResponse(string functionName, string data)
        {
            ...
            if (actions != null)
            {
                Parallel.ForEach(actions, (callback) =>
                {
                    ...
                    {
                        Console.WriteLine($"HandleResponse error: {ex.Message}");
                    }
                });
            }
        }
        ...
        private void SocketClosed()
        {
            ...
            {
                Console.WriteLine($"singlr close,listeners.Count={listeners.Count},reconnecting={reconnecting} +++++++++++++++++++");
                return;
            }
        ...
        private async Task ReconnectLoop()
        {
            if (reconnecting)
            {
                Console.WriteLine($"ReconnectLoop singlr close +++++++++++++++++++");
                return;
            }
            reconnecting = true;
            try
            {
                ...
                while (!disposed && (hubConnection == null || (hubConnection.State != ConnectionState.Connected && hubConnection.State != ConnectionState.Connecting)))
                {
                    ...
                    {
                        await StartAsync();
                        Console.WriteLine($"singlr restart +++++++++++++++++++");
                    }
                    catch
                    {
                        // wait 5 seconds before attempting reconnect
                        await Task.Delay(5000);
                        Console.WriteLine($"singlr reconnect ++++++++++++++++++++++++++++++++++++++++++");
                    }
                }
            }
            catch(Exception ex)
            {
                Console.WriteLine($"singlr reconnect error: {ex.Message}");
            }
            reconnecting = false;
        }
        ...
        {
            Console.WriteLine($"recevice data: {obj} +++++++++++++++++++++++++++++");
        }

        private void Error(Exception obj)
        {
            Console.WriteLine($"has error: {obj.Message} +++++++++++++++++++++++++++++");
        }
        ...
        private void StateChange(StateChange stateChange)
        {
            Console.WriteLine($"{stateChange.OldState.ToString()} change to {stateChange.NewState.ToString()} +++++++++++++++++");
        }
        /// <summary>
